...
class DCAHybridBacktester:
    ...
    def __init__(self, buy_model, sell_model, yearly_capital=600_000):
        ...
        self.ai_buy_signals = []       # AI 買入點 {'date', 'price', 'confidence'}
        self.ai_sell_signals = []      # AI 賣出點 {'date', 'price', 'confidence'}
        ...
        for i in range(len(df)):
            ...
            if ai_position is not None:
                ...
                self.daily_confidence.append({
                    ...
                    'ai_action': 'SELL' if action[0] == 1 else 'HOLD'
                })
                ...
                if should_sell:
                    ...
                    self.trades.append({
                        ...
                        'buy_confidence': ai_position.get('buy_confidence', 0),
                        'sell_date': date,
                        'sell_price': price,
                    ...
                    self.ai_sell_signals.append({'date': date, 'price': price, 'confidence': sell_confidence})
                    ai_position = None
            ...
            # --- AI 空手：檢查買入 ---
                ...
                self.daily_confidence.append({
                    ...
                    'ai_action': 'BUY' if action[0] == 1 else 'WAIT'
                })
                ...
                if action[0] == 1:  # Buy
                    ...
                    if shares > 0:
                        ...
                        self.ai_buy_signals.append({'date': date, 'price': price, 'confidence': buy_confidence})
...
def main():
    ...
    if backtester.ai_buy_signals:
        ai_buy_dates = [s['date'] for s in backtester.ai_buy_signals]
        ai_buy_prices = [s['price'] for s in backtester.ai_buy_signals]
        ai_buy_confs = [s['confidence'] for s in backtester.ai_buy_signals]
        
        ax2.scatter(ai_buy_dates, ai_buy_prices, marker='^', color='red', s=100, label='AI Buy', zorder=5)
        # 標註信心度
        for dt, pr, conf in zip(ai_buy_dates, ai_buy_prices, ai_buy_confs):
    ...
    if backtester.ai_sell_signals:
        ai_sell_dates = [s['date'] for s in backtester.ai_sell_signals]
        ai_sell_prices = [s['price'] for s in backtester.ai_sell_signals]
        ai_sell_confs = [s['confidence'] for s in backtester.ai_sell_signals]
        
        ax2.scatter(ai_sell_dates, ai_sell_prices, marker='v', color='green', s=100, label='AI Sell', zorder=5)
        # 標註信心度
        for dt, pr, conf in zip(ai_sell_dates, ai_sell_prices, ai_sell_confs):
            ax2.annotate(f'{conf:.0%}', (dt, pr), textcoords="offset points", 
    ...
    if backtester.daily_confidence:
        ...
        conf_path = os.path.join(RESULTS_PATH, f'daily_confidence_v4_dca_hybrid_no_filter_{start_str}_{end_str}.csv')
        conf_df.to_csv(conf_path, index=False)
        print(f"[Output] Daily Confidence: {conf_path}")
